...
7. 학생이 자기 팀과 팀원을 확인합니다. 자동 배정에서 빠진 학생은 조별 주제를 보고 직접 고릅니다.
8. 교사가 수업을 종료하면 룸 데이터를 모두 삭제합니다.

## 답변이 부실한 학생 처리

답변 품질에 따라 세 갈래로 나눕니다.

| 상태 | 판정 | 처리 |
|---|---|---|
| 정상·저품질 답변 | 주제에 맞고 답변이 있음 | 임베딩으로 의미 배정 |
| 주제 이탈(장난) | Gemini가 `isOffTopic` 판정 | 자동 배정 제외, 직접 선택 |
| 미제출·빈 답변 | 답변 없음 | 자동 배정 제외, 직접 선택 |

의미 배정은 **답변한 학생 수만큼만** 자리를 씁니다. 20명 5조에서 17명이 답변했다면 팀별 의미
배정 정원은 4·4·3·3·3이 되고, 남는 3자리가 서로 다른 팀에 흩어집니다. 그래서 답변하지 않은
학생이 한 팀에 몰리지 않습니다.

남은 자리는 다음 중 하나로 채웁니다.

- 교사가 검토 화면의 `확인이 필요한 학생` 목록에서 직접 조를 지정
- 확정 후 학생이 `관심 있는 조를 골라 주세요` 화면에서 직접 선택
  (`POST /api/rooms/[code]/result/pick`, 정원이 차면 409로 거부)

분석 가능한 답변이 하나도 없으면 빈 팀만 남지 않도록 전원을 인원 균형으로 배정합니다.
